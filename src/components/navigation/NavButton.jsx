...
export default function NavButton({ x, y, label, link, icon, newTab }) {
  const getIcon = (icon) => {
    switch (icon) {
      ...
      case "support":
        return <Coffee className="w-full h-auto" strokeWidth={1.5} />;
      case "resume":
    ...
    >
      ...
      >
        <span className="relative w-14 h-14 p-4 animate-spin-slow-reverse group-hover:pause hover:text-accent">
          ...
          <span className="absolute hidden peer-hover:block p-2 left-full mx-2 top-1/2 -translate-y-1/2 bg-background text-foreground text-sm rounded-md shadow-lg whitespace-nowrap">
            {label}
          </span>
        </span>
      </Link>
